...
# Original Source: https://github.com/evertramos/docker-compose-letsencrypt-nginx-proxy-companion
#
...
# 4. Download the latest version of nginx.tmpl if file not exist
if [ ! -f "$NGINX_FILES_PATH/nginx.tmpl" ]; then
...
if [ ! -z ${USE_NGINX_CONF_FILES+X} ] && [ "$USE_NGINX_CONF_FILES" = true ]; then
  ...
  cp -R ./config/nginx.conf $NGINX_FILES_PATH/conf.d

  # Check if there was an error and try with sudo
  if [ $? -ne 0 ]; then
      sudo cp -R ./config/nginx.conf $NGINX_FILES_PATH/conf.d
  fi
